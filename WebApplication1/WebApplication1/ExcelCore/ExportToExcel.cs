...
namespace WebApplication1.ExcelCore
{
    public class ExportToExcel : IDisposable
    {
        ...
        {
            this.worksheet = workbook.Worksheets.Add("Nhân viên");
        ...
        public virtual void Export()
        {
            ...
            _response.AddHeader("content-disposition", string.Format("attachment; filename={0}.xlsx", _filename??(new Guid()).ToString()));
    ...
    public class ExportMultiSheet
    {
        public void Export()
        {
            using (XLWorkbook book=new XLWorkbook())
            {
                for (int i = 0; i < 5; i++)
                {
                    var sheet = book.Worksheets.Add("demo "+i.ToString());
                    sheet=this.Set(sheet);
                }
                HttpResponse _response = HttpContext.Current.Response;
                _response.ClearContent();
                _response.Buffer = true;
                _response.AddHeader("content-disposition", string.Format("attachment; filename={0}.xlsx", "demo" ?? (new Guid()).ToString()));
                _response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
                _response.Charset = "";
                using (MemoryStream MyMemoryStream = new MemoryStream())
                {
        ...
        private IXLWorksheet Set(IXLWorksheet worksheet)
        {
            worksheet.Cell(1, 1).Value = "dmeo";
            return worksheet;
        }
    }
}